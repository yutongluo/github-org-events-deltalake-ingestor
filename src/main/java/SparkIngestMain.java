...
public class SparkIngestMain {
  ...
  private void run() {
    ...
    if (isProd) {
      ...
      builder.config("fs.azure.account.auth.type." + storageAccountName + ".dfs.core.windows.net",
              "OAuth")
          .config("fs.azure.account.oauth.provider.type." + storageAccountName
                  + ".dfs.core.windows.net",
              "org.apache.hadoop.fs.azurebfs.oauth2.ClientCredsTokenProvider")
          .config(
              "fs.azure.account.oauth2.client.id." + storageAccountName + ".dfs.core.windows.net",
              applicationId)
          .config("fs.azure.account.oauth2.client.secret." + storageAccountName
                  + ".dfs.core.windows.net",
              password)
          .config("fs.azure.account.oauth2.client.endpoint." + storageAccountName
                  + ".dfs.core.windows.net",
    ...
    final String path =
        isProd ? "abfss://orgevents@" + storageAccountName + ".dfs.core.windows.net" : "/tmp";
  ...
  /**
   * Sometimes GitHub org is not the same as org name in the database. Given the GitHub org name,
   * map to correct name in delta table
   *
